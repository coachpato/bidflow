...
function validTender(reference, overrides = {}) {
  return {
    ...
    raw: {
      ...
      closing_Date: '2026-09-30T00:00:00.000Z',
    },
    ...overrides,
  }
}
    ...
    sourceRun: {
      ...
      findMany: jest.fn(async ({ where }) => runs
        .filter(run =>
          ...
          && (!where.runStatus || run.runStatus === where.runStatus)
        )
...
describe('crawler end-to-end integration scenarios', () => {
  ...
  it('timeout after page three saves cursor but the next scheduled run starts from page one', async () => {
    ...
    expect(secondRunPages).toEqual([1, 2, 3, 4, 5])
  })
