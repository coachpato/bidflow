...
async function sendSubscriberDigestEmail({ subscriber, sourceRun, tenders }) {
  ...
  const listRows = tenders
    .map(tender => {
      const canonicalUrl = tender.canonicalUrl || buildBid360TenderUrl(tender.id, tender.title)
      const sourceUrl = normalizeEtendersDigestUrl(tender.sourceFallbackUrl || ETENDERS_GENERAL_OPPORTUNITIES_URL)
      const tenderLink = canonicalUrl
        ? `<a href="${escapeHtml(canonicalUrl)}" style="color:#18314a;font-weight:700;">${escapeHtml(tender.title)}</a>`
        : `<strong>${escapeHtml(tender.title)}</strong>`
      const sourceLink = `<br/><a href="${escapeHtml(sourceUrl)}" style="font-size:13px;color:#18314a;">View opportunities on eTenders</a>`

      return `
        <tr>
          <td style="padding:12px;border-bottom:1px solid #e5e7eb;">
            ${tenderLink}${sourceLink}<br/>
  ...
  const textTenderRows = tenders.map(tender => [
    ...
    (tender.canonicalUrl || buildBid360TenderUrl(tender.id, tender.title))
      ? `  Link: ${tender.canonicalUrl || buildBid360TenderUrl(tender.id, tender.title)}`
      : null,
    `  Source: View opportunities on eTenders - ${normalizeEtendersDigestUrl(tender.sourceFallbackUrl || ETENDERS_GENERAL_OPPORTUNITIES_URL)}`,
  ].filter(Boolean).join('\n'))
