...
const SECTOR_KEYWORD_CONFIGS = {
  ...
  FINANCIAL_SERVICES: {
    ...
    defaultPracticeArea: 'Financial Services',
  },
  GREEN_ENERGY: {
    categories: {
      core: [
        'renewable energy', 'green energy', 'solar', 'solar pv', 'wind', 'battery energy storage',
        'battery storage', 'energy transition', 'grid integration', 'clean energy', 'climate finance',
      ],
      qualifications: [
        'renewable energy expertise', 'owner engineer', 'independent engineer', 'grid specialist',
        'environmental and social', 'energy modelling',
      ],
      roles: [
        'owner engineer', 'independent engineer', 'grid study', 'resource assessment',
        'energy advisory', 'programme management', 'transaction support', 'project finance',
      ],
      panels: [
        'panel of renewable energy advisors', 'panel of owner engineers', 'green energy services providers',
        'renewable energy consulting services', 'energy transition advisors',
      ],
      services: [
        'solar pv', 'wind energy', 'battery storage', 'grid integration', 'energy storage',
        'environmental and social advisory', 'climate finance', 'energy procurement',
      ],
    },
    practiceAreaKeywords: {
      'Solar PV': ['solar', 'solar pv'],
      'Wind energy': ['wind', 'wind energy'],
      'Battery storage': ['battery energy storage', 'battery storage', 'energy storage'],
      'Grid integration': ['grid integration', 'grid study'],
      'Environmental and social advisory': ['environmental and social', 'environmental and social advisory'],
      'Owner’s engineer': ['owner engineer', 'independent engineer'],
      'Energy programme management': ['programme management', 'energy advisory'],
      'Climate finance support': ['climate finance', 'project finance', 'transaction support'],
    },
    defaultPracticeArea: 'Green Energy Services',
  },
  BUILT_ENVIRONMENT: {
    categories: {
      core: [
...
function getSectorConfig(sector) {
  return SECTOR_KEYWORD_CONFIGS[normalizeServiceSector(sector) || 'LEGAL_SERVICES'] || SECTOR_KEYWORD_CONFIGS.LEGAL_SERVICES
}
...
export function analyzeTenderForSector(sector, title = '', description = '', pdfText = '') {
  const normalizedSector = normalizeServiceSector(sector) || 'LEGAL_SERVICES'
...
export function analyzeForLegalOpportunity(title = '', description = '', pdfText = '') {
  return analyzeTenderForSector('LEGAL_SERVICES', title, description, pdfText)
}

export function getRelevantKeywords(matchedKeywords, limit = 5) {
  return getRelevantKeywordsForSector('LEGAL_SERVICES', matchedKeywords, limit)
}

export function identifyPracticeArea(matchedKeywords) {
  return identifyPracticeAreaForSector('LEGAL_SERVICES', matchedKeywords)
}
