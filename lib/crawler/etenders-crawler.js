...
export async function getTenderDetails(tenderInput) {
  try {
    ...
    return withRetry(
      async () => {
        const response = await axios.get(validatedUrl.url, {
          headers: getDefaultHeaders(),
          timeout: 45000,
        })
      ...
      `Get tender details from ${validatedUrl.url}`,
...
export async function downloadPDF(pdfUrl) {
  try {
    ...
    return await withRetry(
      ...
      `Download PDF from ${validatedUrl.url.substring(0, 80)}`,
      { ...RETRY_CONFIG, maxRetries: 2 }
    )
  } catch (error) {
...
export async function getPDFLinksFromTender(tenderInput) {
  try {
    ...
    return await withRetry(
      ...
      `Get PDF links from ${validatedUrl.url}`,
      { ...RETRY_CONFIG, maxRetries: 1 }
    )
  } catch (error) {
