...
export async function crawlETenders() {
  try {
    console.log('[Crawler] Starting eTenders portal crawl...')
    ...
    while (totalRecords === null || start < totalRecords) {
      try {
        ...
        if (totalRecords === null) {
          ...
          console.log(`[Crawler] Total records to fetch: ${totalRecords}`)
        }

        if (pageRows.length === 0) {
          console.log(`[Crawler] Page ${pagesFetched} returned no data, stopping pagination`)
          break
        }
        ...
        pagesFetched += 1
        console.log(`[Crawler] Page ${pagesFetched}: Fetched ${pageRows.length} rows, mapped ${mappedRows.length}`)
    ...
    console.log(`[Crawler] Successfully crawled ${tenders.length} tenders from eTenders in ${pagesFetched} pages`)
    return tenders
  } catch (error) {
  ...
  try {
    ...
    return await withRetry(
      async () => {
        ...
        console.log(`[PDF] Downloaded ${buffer.length} bytes from ${pdfUrl.substring(0, 80)}...`)
        return buffer
      },
  ...
  try {
    ...
    return await withRetry(
      async () => {
        ...
        console.log(`[PDF] Found ${pdfLinks.length} PDF links on tender page`)
        return pdfLinks
      },
