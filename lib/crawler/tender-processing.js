...
async function uploadPDFToSupabase(fileName, pdfBuffer, opportunityId) {
  try {
    ...
    const normalizedFileName = String(fileName ?? '') || 'tender-document.pdf'
    const sanitizedFileName = normalizedFileName.replace(/[^a-z0-9._-]/gi, '_') || 'tender-document.pdf'
    const filePath = `opportunities/${opportunityId}/${Date.now()}_${sanitizedFileName}`
