...
describe('eTenders row mapping', () => {
  ...
  it('coerces non-string row fields before whitespace cleanup', () => {
    const row = mapOpportunityRow({
      id: 166952,
      tender_No: 12345,
      description: { toString: () => 'Panel   of\nattorneys' },
      category: { toString: () => 'Legal   services' },
      conditions: ['CIDB', 'level'],
      department: { toString: () => 'Justice   Department' },
      province: 9,
      status: { toString: () => 'Open' },
      contactPerson: { toString: () => 'Jane   Doe' },
      email: { toString: () => 'jane@example.test' },
    })

    expect(row.title).toBe('Panel of attorneys')
    expect(row.reference).toBe('12345')
    expect(row.category).toBe('Legal services')
    expect(row.description).toContain('Justice Department')
    expect(row.tenderDetails.contactPerson).toBe('Jane Doe')
    expect(row.sourceStatus).toBe('Open')
  })
