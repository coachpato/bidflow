...
function normalizeSectorToken(value) {
...
export function buildManualMatchData({
  ...
  const legacyDefaultPracticeArea = normalizeSectorToken(serviceSector) === 'ACCOUNTING'
    ? getDefaultPracticeAreaForSector(serviceSector)
    : null
  const defaultPracticeArea = legacyDefaultPracticeArea
    || (normalizedSector ? getDefaultPracticeAreasForSector(normalizedSector)[0] : null)
    || (normalizedSector ? getDefaultPracticeAreaForSector(normalizedSector) : null)
    || 'General Services'
  const reasons = [
...
export function evaluateOpportunityMatch({
  ...
  const serviceSector = normalizeServiceSector(firmProfile?.serviceSector)
  if (!serviceSector) {
    return {
      isMatch: false,
      fitScore: Math.round(tenderAnalysis?.score || 0),
      verdict: 'Weak match',
      matchedKeywords: [],
      practiceArea: 'General Services',
      recommendedStatus: 'Ignore',
      matchReasons: ['No firm sector is set for opportunity matching.'],
