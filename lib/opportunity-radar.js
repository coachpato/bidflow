...
export function buildManualMatchData({
  ...
  const normalizedSector = normalizeServiceSector(serviceSector) || 'LEGAL_SERVICES'
  ...
  const defaultPracticeArea = legacyDefaultPracticeArea
    || getDefaultPracticeAreasForSector(normalizedSector)[0]
...
export function evaluateOpportunityMatch({
  ...
  const serviceSector = normalizeServiceSector(firmProfile?.serviceSector) || 'LEGAL_SERVICES'
