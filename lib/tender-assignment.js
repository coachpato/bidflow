...
export async function notifyTenderAssignees({
  ...
  const message = `${actorName} assigned you to pursuit "${tender.title}".`
  ...
  // Business event: a pursuit was assigned to a new owner after the record update succeeded.
  const emailResults = await Promise.allSettled(
    newRecipients.map(async recipient => {
      const subject = `Pursuit assigned: ${tender.title}`
  const intro = `${escapeHtml(actorName)} assigned you to a pursuit in Bid360.`
    ...
    ? `<p><a href="${tenderUrl}">Open this pursuit in Bid360</a></p>`
        : ''
      ...
      return sendEmail({
        ...
        text: [
          ...
          '',
    `${actorName} assigned you to a pursuit in Bid360.`,
  ...
  for (const result of emailResults) {
    ...
    console.error('Pursuit assignment email failed:', result.reason)
  }
