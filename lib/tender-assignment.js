...
export async function resolveAssignedRecipients(assignedTo) {
...
export async function notifyTenderAssignees({
  ...
  const nextRecipients = await resolveAssignedRecipients(assignedTo)
  if (!nextRecipients.length) return { notifiedUsers: 0, emailedRecipients: 0 }

  const previousRecipients = previousAssignedTo
    ? await resolveAssignedRecipients(previousAssignedTo)
    : []
