...
export async function syncComplianceExpiryNotificationsSafely(organizationId) {
  try {
    return await syncComplianceExpiryNotifications(organizationId)
  } catch (error) {
    console.error('Compliance expiry sync failed:', {
      organizationId,
      error,
    })
    return null
...
export async function syncComplianceExpiryNotificationsIfNeededSafely(organizationId) {
  try {
    return await syncComplianceExpiryNotificationsIfNeeded(organizationId)
  } catch (error) {
    console.error('Compliance expiry sync-if-needed failed:', {
      organizationId,
      error,
    })
    return null
