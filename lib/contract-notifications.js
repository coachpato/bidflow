...
export async function notifyContractAssignees({
  ...
  const title = 'Contract allocated'
  ...
  // Business event: a contract was allocated to a new owner after the record update succeeded.
  ...
  <p>${escapeHtml(actorName)} allocated a contract to you in Bid360.</p>
            <p><strong>Contract:</strong> ${escapeHtml(contract.title)}</p>
            <p><strong>Client:</strong> ${escapeHtml(contract.client || 'Not set')}</p>
            <p><strong>Award date:</strong> ${escapeHtml(formatContractDate(contract.appointmentDate))}</p>
...
${contractUrl ? `<p><a href="${contractUrl}">Open this contract in Bid360</a></p>` : ''}
          </div>
        `,
        text: [
          `Hello ${recipient.name || recipient.email},`,
          '',
    `${actorName} allocated a contract to you in Bid360.`,
          `Contract: ${contract.title}`,
          `Client: ${contract.client || 'Not set'}`,
          `Award date: ${formatContractDate(contract.appointmentDate)}`,
...
export async function sendContractDateReminder({
  ...
  const title = dateField === 'renewalDate' ? 'Contract renewal reminder' : 'Contract end-date reminder'
  ...
  // Business event: a contract end or renewal date entered the reminder window after the reminder scan succeeded.
  const emailResults = await Promise.allSettled(
...
export async function sendAppointmentFollowUpReminder({
  ...
  const linkedUsers = recipients.filter(recipient => recipient.id)
  const title = 'Contract follow-up due'
  const message = `Contract "${contract.title}" needs follow-up ${countdownLabel}.`
  ...
  // Business event: a contract follow-up date entered the reminder window after the reminder scan succeeded.
  const emailResults = await Promise.allSettled(
    recipients.map(recipient => (
      sendEmail({
        to: recipient.email,
        subject: `Contract follow-up due: ${contract.title}`,
        html: `
          <div style="font-family: Arial, sans-serif; color: #0f172a; line-height: 1.5;">
            <p>Hello ${escapeHtml(recipient.name || recipient.email)},</p>
            <p>The contract below needs a follow-up action.</p>
            <p><strong>Contract:</strong> ${escapeHtml(contract.title)}</p>
            <p><strong>Client:</strong> ${escapeHtml(contract.client || 'Not set')}</p>
...
${contractUrl ? `<p><a href="${contractUrl}">Open this contract in Bid360</a></p>` : ''}
        ...
        text: [
          `Hello ${recipient.name || recipient.email},`,
          '',
          'The contract below needs a follow-up action.',
          `Contract: ${contract.title}`,
          `Client: ${contract.client || 'Not set'}`,
  ...
  for (const result of emailResults) {
    ...
    console.error('Contract follow-up email failed:', result.reason)
  }
