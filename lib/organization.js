...
import {
  ...
  getDefaultTargetWorkTypesForSector,
  normalizeServiceSector,
  normalizeServiceSectors,
...
function getDefaultOrganizationName(user, serviceSector = null) {
  ...
  if (user.name?.trim()) {
    ...
    if (normalizedSector === 'FINANCIAL_SERVICES') return `${user.name.trim()} Advisory`
    if (normalizedSector === 'GREEN_ENERGY') return `${user.name.trim()} Energy`
  ...
  if (domain) {
    ...
    if (normalizedSector === 'FINANCIAL_SERVICES') return `${normalizedDomain} Advisory`
    if (normalizedSector === 'GREEN_ENERGY') return `${normalizedDomain} Energy`
...
export function isOrganizationSetupComplete(organizationContext) {
  const organization = organizationContext?.organization || organizationContext
  const firmProfile = organizationContext?.firmProfile || organization?.firmProfile

  if (!organization?.setupCompletedAt) {
    return false
  }

  const displayName = firmProfile?.displayName?.trim()
  const sectors = normalizeServiceSectors(firmProfile?.serviceSectors)

  return Boolean(displayName) && sectors.length > 0
}
...
export async function ensureOrganizationContextForUser(user, options = {}) {
  ...
  const practiceAreas = normalizeList(options.practiceAreas)
  const targetWorkTypes = normalizeList(options.targetWorkTypes)
  const targetProvinces = normalizeList(options.targetProvinces)
  const preferredEntities = normalizeList(options.preferredEntities)
  const serviceSectors = normalizeServiceSectors(options.serviceSectors || [serviceSector].filter(Boolean))
  const defaultPracticeAreas = getDefaultPracticeAreasForSector(serviceSector)
  const defaultTargetWorkTypes = getDefaultTargetWorkTypesForSector(serviceSector)
  const setupCompletedAt = organizationName && serviceSector ? new Date() : null
  ...
  const created = await prisma.$transaction(async tx => {
    ...
    const firmProfile = await tx.firmProfile.create({
      data: {
        ...
        primaryContactName: user.name,
        primaryContactEmail: user.email,
        practiceAreas: practiceAreas.length > 0 ? practiceAreas : defaultPracticeAreas,
        targetWorkTypes: targetWorkTypes.length > 0 ? targetWorkTypes : defaultTargetWorkTypes,
        targetProvinces,
        preferredEntities,
      },
    })
