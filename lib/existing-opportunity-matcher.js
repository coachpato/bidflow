...
function buildBackfilledSummary(match, sourceOpportunity) {
  return [
    sourceOpportunity.summary,
    match.matchReasons?.slice(0, 2).join(' '),
  ].filter(Boolean).join('\n\n')
}

function buildBackfilledNotes(match, sourceOpportunity) {
  ...
  return [
    ...
    ...match.matchReasons,
    keywordLine,
    sourceOpportunity.notes ? `Original notes: ${sourceOpportunity.notes}` : null,
  ].filter(Boolean).join('\n\n')
}
...
async function loadExistingSourceOpportunities(organizationId) {
...
export async function matchExistingOpportunitiesForOrganization(organizationId) {
    ...
    const opportunity = existingOpportunity
      ...
      : await prisma.opportunity.create({
          data: {
            ...
            notes: buildBackfilledNotes(match, sourceOpportunity),
