...
describe('Sector-aware opportunity radar', () => {
  ...
  it('does not fall back to legal matching when no sector is configured', () => {
    const analysis = analyzeTenderForSector(null, 'Panel of attorneys for legal services', '')
  ...
  it('matches accounting keywords as whole words instead of substrings', () => {
    const analysis = analyzeTenderForSector('ACCOUNTING', 'RFQJW091TN26 - AUDITORIUM UPGRADE', '')
  ...
  it('does not treat legal-only generated match text as a financial services signal', () => {
    const analysis = analyzeTenderForSector(
      'FINANCIAL_SERVICES',
