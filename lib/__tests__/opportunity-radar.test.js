...
describe('Sector-aware opportunity radar reference behavior', () => {
  ...
  it('does not treat broad legal words or categories as Legal opportunities', () => {
    const categoryOnly = analyzeTender(
      'LEGAL',
      'Supply and delivery of cleaning materials',
      'Category: Legal and accounting activities. Compliance with municipal procurement requirements.'
    )
    const regulatoryOnly = analyzeTender(
      'LEGAL',
      'Enterprise architecture and ICT strategy for a regulatory authority',
      'The service provider must consider regulatory compliance and governance.'
    )

    expect(categoryOnly.isSectorOpportunity).toBe(false)
    expect(regulatoryOnly.isSectorOpportunity).toBe(false)
  })

  it('uses strict Legal signals for LLB and debt-collection opportunities', () => {
    const llbAnalysis = analyzeTender(
      'LEGAL',
      'Appointment of a service provider for disciplinary hearing services',
      'The lead resource must have an LLB degree and be an admitted attorney.'
    )
    const debtAnalysis = analyzeTender(
      'LEGAL',
      'Panel of debt collectors',
      'Collection of outstanding municipal accounts and recovery of arrear debt.'
    )

    expect(llbAnalysis.isSectorOpportunity).toBe(true)
    expect(llbAnalysis.matchedKeywords).toEqual(expect.arrayContaining(['LLB or admitted legal qualification']))
    expect(debtAnalysis.isSectorOpportunity).toBe(true)
    expect(debtAnalysis.matchedKeywords).toEqual(expect.arrayContaining(['debt collection']))
  })
