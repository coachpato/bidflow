import { normalizeStatusForBadge } from '@/lib/status-compat'
...
const STATUS_STYLES = {
  ...
  Liked: { badge: 'border border-teal-200 bg-teal-50 text-teal-800', dot: 'bg-teal-600' },
  Disliked: { badge: 'border border-rose-200 bg-rose-50 text-rose-800', dot: 'bg-rose-600' },
  Pursued: { badge: 'border border-emerald-200 bg-emerald-50 text-emerald-800', dot: 'bg-emerald-600' },
  ...
  'Not Appointed': { badge: 'border border-rose-200 bg-rose-50 text-rose-800', dot: 'bg-rose-600' },
  ...
  'Work Complete': { badge: 'border border-violet-200 bg-violet-50 text-violet-800', dot: 'bg-violet-600' },
  // Appeal statuses
  Pending: { badge: 'border border-amber-200 bg-amber-50 text-amber-800', dot: 'bg-amber-500' },
...
export default function StatusBadge({ status }) {
  const normalizedStatus = normalizeStatusForBadge(status)
  const style = STATUS_STYLES[normalizedStatus] || { badge: 'border border-slate-200 bg-slate-50 text-slate-700', dot: 'bg-slate-500' }
  return (
