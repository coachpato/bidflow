...
function SettingsNavItem({ item, active }) {
  ...
  const disabledClass = 'cursor-not-allowed border-transparent bg-transparent text-slate-400'

  if (item.disabled) {
    return (
      <button type="button" disabled className={`${baseClass} ${disabledClass}`}>
        <span>{item.label}</span>
        <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">
          Coming soon
        </span>
      </button>
    )
  ...
  return (
    <div className="app-page py-5 sm:py-6">
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-start">
        <aside className="app-surface rounded-[24px] p-3 lg:sticky lg:top-24">
          ...
          <nav aria-label="Settings" className="mt-3 flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible lg:pb-0">
